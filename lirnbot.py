...
# COOKIES
COOKIE_PIXELYA = "s%3AgLVcmK9Zi7AjIb8rAJX2oaGO0dB84GXi.UEhch5q%2BonqeRKk4wcHCx%2BEPsvYualKOJqlhD5qjETM"
COOKIE_PIXMAP = "s%3AS2qBqqlzYPCWST-OalOz6svoEoTYQIi9.%2BL0JZVKMRNrHr9eQ8WAuf4D9MdthKJP3pHCrqliUmZs"
COOKIE_PIXUNIVERS = "s%3Avwp64WhUKpDYEe0xic1bCvf2SwE9jsyu.9jYCpwgpg3rxKCZ7i6i2n0A93NJ0eZH8SqDdNHWRfR"  # ⚠️ ДОДАЙТЕ COOKIE ДЛЯ PIXUNIVERS
...
async def get_map_area(site_name, x, y, w, h):
    ...
    logger.info(f"🗺️ Чанки: X[{cx_start}..{cx_end}], Y[{cy_start}..{cy_end}]")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for cy in range(cy_start, cy_end + 1):
            for cx in range(cx_start, cx_end + 1):
                ...
                tasks.append(process_chunk(session, url, headers, cx, cy, colors, sem, canvas, px, py))
...
def compare_with_template(board):
    """Порівнює з шаблоном"""
    ...
    diff_r = np.abs(t_arr[:,:,0].astype(int) - b_arr[:,:,0].astype(int))
    diff_g = np.abs(t_arr[:,:,1].astype(int) - b_arr[:,:,1].astype(int))
    diff_b = np.abs(t_arr[:,:,2].astype(int) - b_arr[:,:,2].astype(int))
    ...
    logger.info(f"📊 Порівняння: {correct_count}/{total} ({percent:.1f}%)")
...
async def check_cmd(u: Update, c):
    ...
    msg = await u.message.reply_text("⏳ Завантажую карту...")
    ...
    try:
        ...
        board = await get_map_area(state["site"], x, y, w, h)
        result, overlay = compare_with_template(board)
...
async def debug_cmd(u: Update, c):
    ...
    msg = await u.message.reply_text("👁️ DEBUG...")
