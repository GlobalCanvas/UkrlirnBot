...
SITES = {
    ...
    "globepixel": {
        ...
        "chunks_url": "https://globepixel.net/chunks/{canvas_id}/{ix}/{iy}.bmp",
    ...
    for attempt in range(2):
        try:
            async with session.get(url, headers=API_HEADERS, timeout=10) as resp:
                ...
                logger.debug(f"✅ Chunk [{ix},{iy}]")
                return True
                
        except asyncio.TimeoutError:
            logger.warning(f"Timeout chunk [{ix},{iy}]")
        except Exception as e:
            logger.warning(f"Помилка chunk [{ix},{iy}]: {e}")
        
        if attempt < 1:
            await asyncio.sleep(0.3)
    
    logger.warning(f"❌ Failed chunk [{ix},{iy}]")
    return False
...
async def get_canvas_area(canvas_id, canvas_info, x, y, width, height):
    ...
    canvas_colors = {}
    for i, color in enumerate(canvas_info.get('colors', [])):
        if len(color) == 3:
...
async def process_lirn_template(template_path, x, y, canvas_id=0, tolerance=20, overlay_path=None):
    ...
    logger.info(f"📏 Канвас: {canvas_info.get('title', '?')} ({canvas_info.get('size', '?')}x{canvas_info.get('size', '?')})")
